...
int ComputerWord(char * word, int index);
	...
	printf("%s \t", guess);
		...
		if (strlen(guess) == WORD_LENGTH) {
			...
			if (strcmp(guess, word)) {
				if (isWord(guess) && hasWord(guess)) { //checking if it has letters under 5 character and if it is in the word list
					++guesses;
					//TODO: Check guess against word
					if (checkWord(guess)) {
						removeAlpha(guess);
						printf("\t%s\n", alpha);
						//findNewWord(guess, alpha, word); 
					} 
				} else {
...
void removeAlpha(const char * guess) {
	...
	if (guess != NULL) {
		while (guess[i]) {
			pos = strpos(alpha, guess[i]); //pass all alphabet and and the word guess GO TO strpos function
			if (pos >= 0) { 
				alpha[pos] = '_'; //replace the letter with an underscore
			}
			++i;
		}
	}
...
int strpos(const char * str, char search) { 
	int i = 0;
	if (str != NULL) { 
		while (str[i]) { //indx through alphabet or correctWord
			if (str[i] == search) { //if the letter is in the guess at all return the number of the position
				return i; //return the index position of a correct letter but in wrong position, (should be greater than or equal to 0)
			}
			++i;
		}
	}
	return -1; //it returns negative one if the letter is not in the guess
}

bool checkWord(const char * guess) { // find words with the correct characters and poition in fpALL, and find words that contain the correct misplaced characters
	if (strlen(guess) == strlen(word)) {
		...
		strcpy(copy, word); //actual word is coppied onto the the variable name  'copy'
		//Do all correct positions first
		while (copy[i]) { //index through the correct word 
			if (copy[i] == guess[i]) { 
		...
		i = 0;
		while (copy[i]) {
			pos = strpos(copy, guess[i]); //passing the correct word and the guess 
...
int ComputerWord(char * word, int index) {
	int i = 0;
	fseek(fpAll, 0, SEEK_SET); 
	while (i <= index && fgets(word, WORD_LENGTH + 1, fpAll) != NULL) {
		if (strlen(word) == WORD_LENGTH) { 
			++i;
		}
	}
